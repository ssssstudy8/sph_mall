            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a
                  href="javascript:"
                  class="mins"
                  @click="skuNum > 1 ? skuNmu-- : (skuNum = 1)"
                  >-</a
                >
              </div>
              <div class="add">
                <!-- 以前的路由跳转  从A到B 这里再加入购物车进行路由跳转之前 发请求把你购买的产品的信息
                通过请求的形式通知服务器，服务器进行相应的储存 -->
  ...
  methods: {
    ...
    async addShopcar() {
      ...
      try {
        ...
        //4、路由跳转
        //5、在路由跳转的时候还需要将产品的信息带给下一级的路由组件
        //一些简单的数据skuNum,通过query形式给路由组件传递过去
        //产品信息的数据（比较复杂：skuInfo），通过会话存储（不持久，会话结束数据在消失）
        //本地存储|会话存储，一般存储的是字符串
