...
export default {
  ...
  computed: {
    ...
    isOne() {
      return this.searchParams.order.indexOf("1") != -1; //如果要检索的字符串值没有出现，则该方法返回 -1。
    },
    isTwo() {
      return this.searchParams.order.indexOf("2") != -1; //如果要检索的字符串值没有出现，则该方法返回 -1。
    },
    isAsc() {
      return this.searchParams.order.indexOf("asc") != -1; //如果要检索的字符串值没有出现，则该方法返回 -1。
    },
    isDesc() {
      return this.searchParams.order.indexOf("desc") != -1; //如果要检索的字符串值没有出现，则该方法返回 -1。
    },
  ...
  methods: {
    ...
    //删除分类名字
    removeCategoryName() {
      //带给服务器参数说明可有可无的：如果当前属性值为空的字符串还是会把相应字段带给服务器
      //但是你把相应的字段变成undefined 当前字段不会带给服务器
    ...
    //排序操作
    changeOrder(flag) {
      //flag形参：他是一个标记 代表用户点击的是综合1 还是价格2 （用户点击的时候传进来）
  ...
  watch: {
    ...
    $route(newValue, oldValue) {
      ...
      //再次发送ajax请求
      this.getData();
      //每次请求完毕 应该把相应的1、2、3级分类的id置空 让他接受下一次的相应的1、2、3级id
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
    },
